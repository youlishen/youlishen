...
public final class SearchContactViewHolder extends ViewHolder implements OnClickListener {
  ...
  private static final String TAG = "SearchContactViewHolder";
  ...
  private final TextView messageToActionView;
  private final ImageView expendView;
  private final View expendViewLayout;
  private final TextView orderHintView;
  private final TextView orderNumberView;
  ...
  public SearchContactViewHolder(View view, RowClickListener listener) {
    ...
    callToActionView = view.findViewById(R.id.wu_liu_call_to_action);
    messageToActionView = view.findViewById(R.id.wu_liu_messaging_to_action);
    messageToActionView.setVisibility(View.VISIBLE);
    messageToActionView.setOnClickListener(this);
    expendView = view.findViewById(R.id.wu_liu_expand);
    expendView.setOnClickListener(this);
    expendViewLayout = view.findViewById(R.id.wu_liu_expand_layout);
    orderHintView = view.findViewById(R.id.wu_liu_order_hint);
    orderNumberView = view.findViewById(R.id.wu_liu_order_number);
  }
  ...
  public void bind(SearchCursor cursor, String query) {
    ...
    String secondaryInfo =
        TextUtils.isEmpty(label)
            ? number
            : context.getString(
                com.android.dialer.R.string.call_subject_type_and_number, label, number);
    ...
    int index = cursor.getColumnIndex(WuLiuContant.COLUMN_NAME_ORDER_INFO);
    String orderNumber = null;

    if (index >= 0) {
      orderNumber = cursor.getString(index);
    }
    LogUtil.d(TAG, "bind index=" +index + "; orderNumber=" + orderNumber);
    if (TextUtils.isEmpty(orderNumber)) {
      orderNumberView.setVisibility(View.GONE);
      orderHintView.setVisibility(View.GONE);
    } else {
  ...
  private void setCallToAction(SearchCursor cursor, String query) {
    ...
    switch (currentAction) {
      ...
      case CallToAction.SHARE_AND_CALL:
        callToActionView.setVisibility(View.VISIBLE);
        callToActionView.setCompoundDrawables(
            context.getDrawable(R.drawable.ic_phone_attach), null, null,null);
      ...
      case CallToAction.VIDEO_CALL:
        callToActionView.setVisibility(View.VISIBLE);
        callToActionView.setCompoundDrawables(
            context.getDrawable(R.drawable.quantum_ic_videocam_white_24), null, null,null);
        callToActionView.setContentDescription(
  ...
  public void onClick(View view) {
    ...
    } else if (view == expendView) {
      if (expendViewLayout.getVisibility() != View.VISIBLE) {
        expendViewLayout.setVisibility(View.VISIBLE);
      } else {
        expendViewLayout.setVisibility(View.GONE);
      }
    } else if (view == messageToActionView) {
      listener.sendMessage(number);
    }
    // else {
    //  listener.placeVoiceCall(number, position);
    //}
