...
public class ProximitySensor
 ...
 //-Jeff_weisy [S] 20190411 SPEAKERISSUE ask by sound part
  private boolean incallstate=false;
  private final PowerManager.WakeLock sPartialWakeLock;
//-Jeff_weisy [E] 20190411 SPEAKERISSUE ask by sound part

 private static boolean debug=false;//ProximitySensor debug remove Jeff_weisy 20190614
...
//-Jeff_weisy [S] 20190411 SPEAKERISSUE ask by sound part
     sPartialWakeLock =powerManager.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, "SPEAKISSUES");
//-Jeff_weisy [S] 20190411 SPEAKERISSUE ask by sound part
    } else {
      LogUtil.i("ProximitySensor.constructor", "Device does not support proximity wake lock.");
      proximityWakeLock = null;
      sPartialWakeLock=null;//-Jeff_weisy [S] 20190411 SPEAKERISSUE ask by sound part
    }
 ...
 void  GetCall_WakeLock()
{
	...
	if(!sPartialWakeLock.isHeld())
	{
	    ...
	    if(debug)
		LogUtil.i("SPEAKISSUES.GO", "sPartialWakeLock.acquire");
	}
     } 
     else
    {     
          if(debug)
           LogUtil.i("SPEAKISSUES.GO", "sPartialWakeLock already acquired");
    }
    
 }
 void ReleaseCall_WakeLock()
 {
      if (sPartialWakeLock != null && sPartialWakeLock.isHeld())
      {
            sPartialWakeLock.release();
	if(debug)
	    LogUtil.i("SPEAKISSUES.GO", "sPartialWakeLock.release");
      }
     else
     {
	if(debug)
	  LogUtil.i("SPEAKISSUES.GO", "sPartialWakeLock already released");
     }
 }
 //-Jeff_weisy [E] 20190411 SPEAKERISSUE ask by sound part
...
//-Jeff_weisy [S] 20190411 SPEAKERISSUE ask by sound part
	...
	incallstate=false;
    //LogUtil.i( "SPEAKISSUES.GO","incallstate: %b ", incallstate);
//-Jeff_weisy [S] 20190411 SPEAKERISSUE ask by sound part     
  ...
  private synchronized void updateProximitySensorMode() {
    ...
    //screenOnImmediately |= dialpadVisible && horizontal;//wanghongjian during call show dialpad the proximity sensor no function
...
//-Jeff_weisy [S] 20190411 SPEAKERISSUE ask by sound part
     if(incallstate&&CallAudioState.ROUTE_SPEAKER == audioRoute)
       {
            if(debug)
		  LogUtil.i("SPEAKISSUES.GO", "ROUTE_SPEAKER");
	   	GetCall_WakeLock();
       }else
       {
             if(debug)
               LogUtil.i("SPEAKISSUES.GO", "ROUTE_SPEAKER not");
	 	ReleaseCall_WakeLock();
       }
//-Jeff_weisy [S] 20190411 SPEAKERISSUE ask by sound part
    Trace.endSection();
  }
