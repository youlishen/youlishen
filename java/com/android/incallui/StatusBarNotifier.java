...
//Jeff_weisy [e] add 20190601 BugID=54306 
  ...
  private void buildAndSendNotification(
      CallList callList, DialerCall originalCall, ContactCacheEntry contactInfo) {
        // yangkun add start 2020/09/15
	if (isYTO) {
            boolean isCallWaiting = CallList.getInstance().getActiveCall() != null &&
...
//Jeff_weisy [e] add 20190601 BugID=54306 
  ...
  private CharSequence getContentString(DialerCall call, @UserType long userType) {
    ...
    if (isIncomingOrWaiting) {
      ...
      }//Jeff_weisy 20190422  for SQ53 customer request to display
 ...
 //Jeff_weisy 20190505  [S] phoneAccount.getLabel() locale change 
  private String getLocalString(String originalString) {
        String[] origNames = context.getResources().
               getStringArray(R.array.original_carrier_names);
        String[] localNames = context.getResources().
                getStringArray(R.array.locale_carrier_names);
        for (int i = 0; i < origNames.length; i++) {
            if (origNames[i].equalsIgnoreCase(originalString)) {
                return context.getResources().getString(
                        context.getResources().getIdentifier(localNames[i],
                        "string", context.getPackageName()));
            }
        }
        return originalString;
  }
  //Jeff_weisy 20190505  [E] phoneAccount.getLabel() locale change 
